...
public interface UmLoginDao {
    @Select("select id from web_mall_building.um_login where password=#{password}")
    Integer queryLoginIdByPassword(String password);

    @Select("select salt from web_mall_building.um_login where id=#{id}")
    String querySaltById(Integer id);

    @Select("select * from web_mall_building.um_login where id=#{id} and password=#{password}")
